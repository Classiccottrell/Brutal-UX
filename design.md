...
13. Model syntax cheat sheet

Per-engine translation of the master prompt blocks in Section 7. Same constraint system, different surface syntax.

| Engine | Strategy | Parameters |
|---|---|---|
| Midjourney v6 | Use the compact token block (§7A). Lean on hard boundary and texture keywords. | `--v 6.1 --style raw --ar 2:3 --stylize 150` (low stylize preserves graphic rigidity) |
| Flux.1 (Dev) | Use the natural-language master block (§7B). Spell out spatial relationships explicitly. | High spatial detail, natural language; instruct literal in-image text explicitly rather than relying on the model to typeset it. |
| DALL-E 3 | Use the natural-language master block (§7B). Call the medium "graphic design" or "risograph print," not "photorealistic." | Explicitly request "flat screenprint reproduction" and "ordered dither," and append the negative block (§7C) as excluded qualities. |
